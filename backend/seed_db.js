...
pool.connect(async (err, client, release) => {
  ...
  try {
    ...
    const filiereRes = await client.query("SELECT id, nom FROM filieres WHERE nom ILIKE '%Réseaux%' LIMIT 1");
    if (filiereRes.rows.length > 0) {
      const filiereId = filiereRes.rows[0].id;
      const filiereNom = filiereRes.rows[0].nom;
      await client.query("UPDATE etudiants SET filiere_id = $1, filiere_nom = $2 WHERE matricule = '24IST-O2/1851'", [filiereId, filiereNom]);
