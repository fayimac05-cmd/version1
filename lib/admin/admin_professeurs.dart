...
final List<Professeur> adminProfesseurs = [
  Professeur(id: 'P001', nom: 'OUÉDRAOGO', prenoms: 'Mamadou', email: 'mamadou.ouedraogo@ist.bf', telephone: '70123456', specialite: 'Réseaux & Systèmes', typeContrat: 'Permanent', dateArrivee: '01/09/2020', noteEvaluation: 4.2),
  Professeur(id: 'P002', nom: 'SAWADOGO', prenoms: 'Issa', email: 'issa.sawadogo@ist.bf', telephone: '76234567', specialite: 'Développement Web', typeContrat: 'Permanent', dateArrivee: '01/09/2021', noteEvaluation: 4.5),
  Professeur(id: 'P003', nom: 'COMPAORÉ', prenoms: 'Brahima', email: 'brahima.compaore@ist.bf', telephone: '65345678', specialite: 'Électrotechnique', typeContrat: 'Vacataire', dateArrivee: '15/01/2022', noteEvaluation: 3.8),
  Professeur(id: 'P004', nom: 'TRAORÉ', prenoms: 'Alassane', email: 'alassane.traore@ist.bf', telephone: '70456789', specialite: 'Cybersécurité', typeContrat: 'Permanent', dateArrivee: '01/09/2019', noteEvaluation: 4.7),
  Professeur(id: 'P005', nom: 'OUÉDRAOGO', prenoms: 'Aïcha', email: 'aicha.ouedraogo@ist.bf', telephone: '76567890', specialite: 'Marketing Digital', typeContrat: 'Vacataire', dateArrivee: '01/02/2023', noteEvaluation: 4.0),
];
...
class _AdminProfesseursState extends State<AdminProfesseurs> {
  ...
  List<Professeur> get _filtered => adminProfesseurs.where((p) {
        ...
        final matchQ = q.isEmpty || p.nom.toLowerCase().contains(q) || p.prenoms.toLowerCase().contains(q) || p.specialite.toLowerCase().contains(q);
        final matchF = _filtre == 'tous' || (_filtre == 'permanent' && p.typeContrat == 'Permanent') || (_filtre == 'vacataire' && p.typeContrat == 'Vacataire');
  ...
  Color _getAvatarColor(String nom) {
    final colors = [Colors.purple, Colors.blue, Colors.teal, Colors.deepOrange, Colors.indigo];
    return colors[nom.length % colors.length];
  }
  ...
  Widget _carteProf(Professeur p) {
    ...
    final avatarColor = _getAvatarColor(p.nom);
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(p.specialite, style: const TextStyle(fontSize: 13, color: Color(0xFF6B7280))),
            const SizedBox(height: 6),
            Row(children: [
              const Icon(Icons.phone_rounded, size: 12, color: Color(0xFF9CA3AF)),
              const SizedBox(width: 4),
              Text(p.telephone, style: const TextStyle(fontSize: 12, color: Color(0xFF6B7280))),
              const SizedBox(width: 12),
              const Icon(Icons.book_rounded, size: 12, color: Color(0xFF9CA3AF)),
              const SizedBox(width: 4),
              Text('${modules.length} module(s)', style: const TextStyle(fontSize: 12, color: Color(0xFF6B7280))),
            ]),
          ])),
        ...
        builder: (ctx, setS) => Container(
          ...
          child: Column(children: [
            ...
            Padding(padding: const EdgeInsets.fromLTRB(20, 14, 20, 0), child: Row(children: [
              const Expanded(child: Text('Ajouter un professeur', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w800, color: Color(0xFF1A1A2E)))),
            ...
            Expanded(child: SingleChildScrollView(padding: const EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(children: [Expanded(child: _fieldCol('Nom *', nomCtrl, 'Ex: OUÉDRAOGO')), const SizedBox(width: 12), Expanded(child: _fieldCol('Prénom *', prenomCtrl, 'Ex: Mamadou'))]),
              const SizedBox(height: 10),
              Row(children: [Expanded(child: _fieldCol('Email *', emailCtrl, 'email@ist.bf', type: TextInputType.emailAddress)), const SizedBox(width: 12), Expanded(child: _fieldCol('Téléphone *', telCtrl, '70000000', type: TextInputType.phone))]),
              const SizedBox(height: 16),
              _sectionHeader('Informations académiques'),
              const SizedBox(height: 12),
              _label('Spécialité *'), _inputField(specCtrl, 'Ex: Réseaux & Systèmes...'),
              const SizedBox(height: 10),
              _label('Date d\'arrivée'), _inputField(dateCtrl, 'Ex: 01/09/2024'),
              const SizedBox(height: 12),
            ...
            Padding(padding: const EdgeInsets.all(16), child: GestureDetector(
              onTap: () {
                if (nomCtrl.text.isEmpty || prenomCtrl.text.isEmpty || emailCtrl.text.isEmpty) { _snack('Nom, prénom et email requis.'); return; }
                setState(() => adminProfesseurs.add(Professeur(id: 'P${adminProfesseurs.length + 1}', nom: nomCtrl.text.toUpperCase(), prenoms: prenomCtrl.text, email: emailCtrl.text, telephone: telCtrl.text, specialite: specCtrl.text, typeContrat: contrat, dateArrivee: dateCtrl.text.isEmpty ? '01/05/2025' : dateCtrl.text)));
                Navigator.pop(ctx);
  ...
  void _ouvrirFiche(Professeur p) {
    ...
    showModalBottomSheet(context: context, isScrollControlled: true, backgroundColor: Colors.transparent, builder: (_) => Container(height: MediaQuery.of(context).size.height * 0.85, decoration: const BoxDecoration(color: Color(0xFFF5F7FA), borderRadius: BorderRadius.vertical(top: Radius.circular(20))), child: Column(children: [
      ...
      Expanded(child: SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _section('Coordonnées', [_infoRow(Icons.email_rounded, p.email), _infoRow(Icons.phone_rounded, p.telephone)]),
        const SizedBox(height: 12),
        _section('Charge académique', [if (modules.isEmpty) const Text('Aucun module assigné.', style: TextStyle(fontSize: 13, color: Color(0xFF6B7280))) else ...modules.map((m) => Padding(padding: const EdgeInsets.symmetric(vertical: 6), child: Row(children: [Container(width: 6, height: 6, decoration: const BoxDecoration(color: AdminTheme.primary, shape: BoxShape.circle)), const SizedBox(width: 10), Expanded(child: Text(m.nom, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Color(0xFF1A1A2E))))])))])
      ])))
    ])));
  }
